...
int main() {
    ...
    kernel_source = (char*)malloc(SOURCE_SIZE);
    kernel_source_size = fread(kernel_source, 1, SOURCE_SIZE, f);
    fclose(f);
    // char logging[SOURCE_SIZE];
    ...
    cl_mem depth_buff_mem = clCreateBuffer(context, CL_MEM_READ_WRITE, WIDTH*HEIGHT*sizeof(float), NULL, &ret);
    cl_mem screen_mem = clCreateBuffer(context, CL_MEM_READ_WRITE, WIDTH*HEIGHT, NULL, &ret);
