...
FT_Library freetype;
std::map<std::string, std::map<uint64_t, Glyph >> fonts = std::map < std::string, std::map < uint64_t, Glyph >> ();
#ifdef USE_CACHE
...
void loadFont(const char* filepath, const char* fontName, int size, uint64_t chars) {
	...
	fonts.insert(std::make_pair(fontName, std::map<uint64_t, Glyph>()));
...
void drawString(std::string string, const char* font, float scale, float x, float y, void (*callEveryChar)()) {
	glBindVertexArray(_getGLObject("Square VAO"));

	float xPos = x;
	//float yPos = y;
	for (std::string::const_iterator c = string.begin(); c != string.end(); c++) {
		Glyph glyph = fonts[std::string(font)][*c];
